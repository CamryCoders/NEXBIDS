...
<>
 ...
 <h2 class="text-2xl font-black tracking-tight text-slate-800">Mint New Auction</h2>
           ...
           className="w-8 h-8 rounded-xl  hover:bg-red-200 transition-colors duration-300">
            <img src="https://img.icons8.com/?size=48&id=T9nkeADgD3z6&format=png" alt="" />
           </div>
          </div>

          <form id="createAuctionForm" action="#" method="POST" class="space-y-5" onSubmit="event.preventDefault();">
...
<details>
  <summary className="bg-slate">Category</summary>
</details>
            ...
            <div class="space-y-1.5">
             ...
             <div class="space-y-1.5 flex justify-between">
                <label for="item-startingPrice" class="block text-xs font-bold text-slate-500 uppercase tracking-wider">Starting Price</label>
                <input
                readOnly={clicked}
                 value={startingPrice}
                onChange={((e)=>{
                ...
                type="number" id="item-model" placeholder="e.g 2400" required class="w-3/4 px-4 py-3 bg-slate-50/60 border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 focus:outline-none focus:border-indigo-500 focus:bg-white transition-all" />
<div className="w-1/4 h-full ml-3">
<label For="hours" className={`text-sm md:text:lg ${startingPrice>=15000?"":"hidden"}`}>Hours Later</label>
              <select
              
              onClick={((e)=>{
                setLater(e.target.value)
               
              })}
              name="hours" className={` w-full border border-violet-400 ${startingPrice>=15000?"":"hidden"} rounded-xl text-sm md:text-lg`}>
  <option value="0" className="bg-slate-100 font-semibold ">Now</option>
  <option value="1" className="bg-violet-100 font-semibold ">1 hours</option>
  <option value="4" className="bg-violet-200 font-semibold ">4 hours</option>
  <option value="6" className="bg-violet-300 font-semibold ">6 hours</option>
  <option value="8" className="bg-violet-400 font-semibold ">8 hours</option>
  <option value="16" className="bg-violet-500 font-semibold ">16 hours</option>
...
{success?<><div class="mx-auto w-12 h-12 rounded-full bg-emerald-50 text-emerald-500 flex items-center justify-center border border-emerald-100 shadow-sm">
              <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
            </div>
